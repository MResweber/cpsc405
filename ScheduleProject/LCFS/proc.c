...
// Added these defines for the schedule latency and 
//    minimum granularity.
#define SCHED_LATENCY 100.0
#define MIN_GRAN 10.0
...
// Added array holding nice values
...
// Otherwise return 0.
static struct proc*
allocproc(int nice) // added nice paramter
{
  struct proc *p;
...
{
  struct proc *p;
  p = allocproc(0); // This function needs a parameter
  initproc = p;
  p->sz = PGSIZE;
  strcpy(p->cwd, "/");
...
int
Fork(int fork_proc_id, int nice) // Added nice paramter
{
  int pid;
  ...
  // Allocate process.
  if((np = allocproc(nice)) == 0) // Added paramter to function call
    return -1;
...
// Changed this so that it follows a LCFS algorithm
  ...
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
    ...
    if (p->state == RUNNABLE || p->state == RUNNING) {
      weight += niceWeights[p->niceness + 20];
      printf("pid: %d weight: %dms\n", p->pid, weight);
    }
  }
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
    if (p->state == RUNNABLE || p->state == RUNNING) {
      float timeslice = ((float)niceWeights[p->niceness+20]/(float)weight) * SCHED_LATENCY;
      if (timeslice < MIN_GRAN) timeslice = MIN_GRAN;
      printf("pid: %d time slice: %.0fms weight: %d\n", p->pid, timeslice, niceWeights[p->niceness+20]);
    }
  }
}
...
// I made it so that this also prints the nice values.
void
procdump(void)
{
  struct proc *p;
